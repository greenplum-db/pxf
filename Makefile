...
stage:
	...
	if [ -n "$(NATIVE_LIBRARY_PATH)" ] && [ -f "$(NATIVE_LIBRARY_PATH)/libtcnative-1.so" ]; then \
		mkdir -p build/stage/$${PXF_PACKAGE_NAME}/pxf/application/native && \
		cp $(NATIVE_LIBRARY_PATH)/libtcnative-1.so build/stage/$${PXF_PACKAGE_NAME}/pxf/application/native/; \
	fi
...
rpm:
	...
	if [ -n "$(NATIVE_LIBRARY_PATH)" ] && [ -f "$(NATIVE_LIBRARY_PATH)/libtcnative-1.so" ]; then \
		...
		cp $(NATIVE_LIBRARY_PATH)/libtcnative-1.so build/rpmbuild/SOURCES/application/native/; \
	fi; \
	rpmbuild \
...
deb:
	...
	if [ -n "$(NATIVE_LIBRARY_PATH)" ] && [ -f "$(NATIVE_LIBRARY_PATH)/libtcnative-1.so" ]; then \
		...
		cp $(NATIVE_LIBRARY_PATH)/libtcnative-1.so build/debbuild/usr/local/pxf-gp$${GP_MAJOR_VERSION}/application/native/; \
	fi; \
