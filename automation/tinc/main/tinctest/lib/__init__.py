...
def get_logger():
    ...
    # Setup info log file
    info_logfile = '%s/tinctest_%s.log' % (logdir, CURRENT_TIME_STRING)
    info_file_handler = logging.FileHandler(info_logfile, 'w')
    info_file_handler.setLevel(logging.INFO)
    info_file_handler.setFormatter(formatter)
    
    # Setup critical/error/warning log file
    cew_logfile = '%s/tinctest_%s_cew.log' % (logdir, CURRENT_TIME_STRING)
    cew_file_handler = logging.FileHandler(cew_logfile, 'w')
    cew_file_handler.setLevel(logging.WARNING)
    cew_file_handler.setFormatter(formatter)
    ...
    # Add all the handlers to logger
    logger.addHandler(info_file_handler)
    logger.addHandler(cew_file_handler)
