...
SELECT type_long, type_string, type_array, type_union, type_enum, type_fixed from avrotest_complex ORDER BY type_long;

CREATE view avro_view
as select type_string, string_to_array(substring(type_map from 2 for (char_length(type_map) - 2)),',')::text[]
as type_map, string_to_array(substring(type_record from 2 for (char_length(type_record) - 2)),',')::text[] as type_record from avrotest_complex;
