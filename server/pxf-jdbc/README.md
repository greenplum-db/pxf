...
If an option is set, it overrides configuration parameter with the same name (except for special cases). Some settings can be set only in configuration file or only in `LOCATION` clause of external table DDL.
...
* **Option**: `BATCH_SIZE`
* **Configuration parameter**: `jdbc.batch.size`
* **Value**: Integer >= 0
...
* **Option**: `POOL_SIZE`
* **Configuration parameter**: `jdbc.pool.size`
* **Value**: Integer
