...
If an option is set, it overrides configuration parameter with the same name. Some settings can be set only in configuration file or only in `LOCATION` clause of external table DDL.
...
Environment variables to set for a session where SELECT or INSERT query generated by PXF are executed. This setting is described in section [external database session configuration](#external-database-session-configuration-1).

* **Configuration parameter**: `jdbc.env`
* **Value**: Key-value pairs, where keys are divided from values by `=` and pairs are divided by `,`. Keys and values must meet the following requirements:
    * Both key and value must not contain `,`
