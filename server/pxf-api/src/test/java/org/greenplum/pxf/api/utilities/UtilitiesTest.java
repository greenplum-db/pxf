...
public class UtilitiesTest {
    ...
    public void testToCsvHandlesNullInputs() {
        String input = "a";
        assertSame(input, Utilities.toCsvText(input, '"', '"', null, ',', true, true, true));
        assertSame(input, Utilities.toCsvText(input, '"', '"', "\n", null, true, true, true));
        assertSame(input, Utilities.toCsvText(input, '"', '"', null, null, true, true, true));
    }
    ...
    public void testToCsvTestInputNewline() {
        ...
        assertEquals("\"\n\"", Utilities.toCsvText(newline1, '"', '"', "\n", ',', true, true, true));
        assertEquals("\"\r\"", Utilities.toCsvText(newline2, '"', '"', "\r", ',', true, true, true));
        assertEquals("\"\r\n\"", Utilities.toCsvText(newline3, '"', '"', "\r\n", ',', true, true, true));
    }
    ...
    public void testToCsvTextStringWithCharacterThatNeedToBeQuoted() {
        ...
        assertSame(input1, Utilities.toCsvText(input1, '"', '"', "\n", ',', true, true, true));

        String input2 = "\""; // quote
        assertEquals("\"\"\"\"", Utilities.toCsvText(input2, '"', '"', "\n", ',', true, true, true));

        String input3 = ","; // delimiter
        assertEquals("\",\"", Utilities.toCsvText(input3, '"', '"', "\n", ',', true, true, true));

        String input4 = "\n\",sample"; // newline, quote and delimiter
        assertEquals("\"\n\"\",sample\"", Utilities.toCsvText(input4, '"', '"', "\n", ',', true, true, true));
    }
    ...
    public void testToCsvTextStringWithoutQuoteCharacter() {
        ...
        assertSame(input, Utilities.toCsvText(input, '|', '"', "\r\n", ',', false, false, false));
        assertEquals("|" + input, Utilities.toCsvText(input, '|', '"', "\r\n", ',', true, false, false));
        assertEquals(input + "|", Utilities.toCsvText(input, '|', '"', "\r\n", ',', false, true, false));
        assertEquals("|" + input + "|", Utilities.toCsvText(input, '|', '"', "\r\n", ',', true, true, false));
    }
    ...
    public void testToCsvDoesNotAddPrefixAndSuffixWhenSkipIfQuotingIsNotNeeded() {
        ...
        assertSame(input, Utilities.toCsvText(input, QUOTE, ESCAPE, NEWLINE, DELIMITER, false, false, true));
        assertSame(input, Utilities.toCsvText(input, QUOTE, ESCAPE, NEWLINE, DELIMITER, false, true, true));
        assertSame(input, Utilities.toCsvText(input, QUOTE, ESCAPE, NEWLINE, DELIMITER, true, false, true));
        assertSame(input, Utilities.toCsvText(input, QUOTE, ESCAPE, NEWLINE, DELIMITER, true, true, true));
    }
    ...
    public void testToCsvTextEscapesQuoteChar() {
        ...
        assertEquals(quoteChar + expected, Utilities.toCsvText(input, quoteChar, escapeChar, "\r\n", ',', true, false, false));
        assertEquals(expected, Utilities.toCsvText(input, quoteChar, escapeChar, "\r\n", ',', false, false, false));
        assertEquals(expected + quoteChar, Utilities.toCsvText(input, quoteChar, escapeChar, "\r\n", ',', false, true, false));
        assertEquals(quoteChar + expected + quoteChar,
                Utilities.toCsvText(input, quoteChar, escapeChar, "\r\n", ',', true, true, false));
    }
